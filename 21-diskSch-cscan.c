...
int main() {
    ...
    // +3 for possible 0, disk_size and returning to 0
    int *sequence = (int *)malloc((n + 3) * sizeof(int));
    int *seek_distances = (int *)malloc((n + 3) * sizeof(int));
    int sequence_size = 0;
    int current_track = head_start;
    ...
    if(direction == 'r') {  // Moving right
        ...
        // If there are tracks to the left
        if(index > 0) {
            // Go to the end
            ...
            seek_distances[sequence_size] = 0;  // Instant return in C-SCAN
            current_track = 0;
            sequence_size++;
            
            // Service tracks from beginning
        ...
        // If there are tracks to the right
        if(index < n) {
            // Go to the beginning
            sequence[sequence_size] = 0;
